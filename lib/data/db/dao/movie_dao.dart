...
abstract class MovieDao extends ContentDao {
  ...
  @Query("SELECT c.* FROM content c JOIN movie m ON c.id = m.id WHERE m.is_popular = 1 ORDER BY m.`order`")
  Stream<List<ContentDb>> observeAllPopular();

  @Query("SELECT c.* FROM content c JOIN movie m ON c.id = m.id WHERE m.is_popular = 0 ORDER BY m.`order`")
